...
- **Web 设置界面**：侧边栏「设置」可配置多实例（Lucky 地址 / OpenToken / 协议 / 采集模块）、测试连接、**手动采集（单实例/全部）与采集进度**（当前源/页数/已采条数，自动轮询）、**本地数据占用**（DB 大小/各表行数/每实例数据量）、全局设置（采集间隔 / 自动清理），保存后采集器热生效且**多实例并发采集（HTTP 并发≤2）**
