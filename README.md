...
- **实时连接详情**：点击「连接数」KPI 即时拉取当前连接（非 30s 快照），按服务分组展示访问者 IP 明细（连接/流量/归属地），带 10s 冷却防抖；每行预留「一键拉黑」按钮（功能即将推出）
- **自动刷新静默化 + 右上角通知**：定时自动刷新不再整页重载闪烁，数据变化时右上角弹窗提示；全局通知机制（notify.js + ToastHost）供所有模块接入（约定见 AGENTS.md）
...
| [doc/11-通知与实时连接设计.md](doc/11-通知与实时连接设计.md) | 自动刷新静默化/全局通知、趋势横轴匹配、实时连接详情设计 |
